%% CHW01 - Computational Intelligence - Question.2.b
...
%% Part.2.b.1 - one hidden layer - two output - not binary values classifying the data
clc; close all;

train_input = NewTrainData(1:3,:);
train_outputNew = [NewTrainData(4,:) ; ~NewTrainData(4,:)];

val_input = ValData(1:3,:);
val_outputNew = [ValData(4,:) ; ~ValData(4,:)];
...
% on train data
net = train(net,train_input,train_outputNew);
yEstimateTrain = net(train_input);

% make the estimates binary with a threshold
threshold = 0.5; 
yEstimateTrainBinary = zeros(2,size(yEstimateTrain,2));
yEstimateTrainBinary(yEstimateTrain >= threshold) = 1;
yEstimateTrainBinary(yEstimateTrain < threshold) = 0;
MSEOnTrainData = perform(net,yEstimateTrainBinary(1,:),train_outputNew(1,:))

% on val data
yEstimateVal = net(val_input);

% make the estimates binary with a threshold
yEstimateValBinary = zeros(2,size(yEstimateVal,2));
yEstimateValBinary(yEstimateVal >= threshold) = 1;
yEstimateValBinary(yEstimateVal < threshold) = 0;
MSEOnValData = perform(net,yEstimateValBinary(1,:),val_outputNew(1,:))

%% Part.2.b.2 - PLOT MSEs of the trained classifier
clc; close all;
...
%% Part.2.b.3 - Apply the classifier on the test data
clc; close all;
...
% make the estimates binary with a threshold
threshold = 0.5;
yEstimateTestBinary = zeros(2,size(TestData,2));
yEstimateTestBinary(yEstimateTest >= threshold) = 1;
yEstimateTestBinary(yEstimateTest < threshold) = 0;
yEstimateTestBinary = ~yEstimateTestBinary;

% seperate two groups
TestDataZeros = TestData(:,find(yEstimateTestBinary(1,:) == 1));
TestDataOnes = TestData(:,find(yEstimateTestBinary(2,:) == 1));
...
% one in each row shows the class, if the first row is one, the data is
% class 0 and if the second row is one, the data is class1 